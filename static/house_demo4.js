 ...
 d3.json(query_url, function(data) {
    ...
    info.onAdd = function (map) {
        ...
        this._div.innerHTML = '<h4> Election Results by House District</h4>' +  (props ?
            '<b>' + props.district + "<br>" + `</b> Pct of ${props.district} that is ${race}:<br />` + parseFloat(props[`${winner}`]).toFixed(2) + '%</b> <br />' 
            : 'Hover over a state');
            };
    if (new_category == "white_pct") {
        winner = "pct_white";
        race = "white";
        new_category = "white_pct"; 
    }else //if (category == white_non_college_pct) 
