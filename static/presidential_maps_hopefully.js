...
var query_url = "http://127.0.0.1:5000/senate_api"
  ...
  d3.json(query_url, function(data) {
    ...
    this._div.innerHTML = '<h4>State Election Results</h4>' +  (props ?
        '<b>' + props.name + `</b><br>  ${category} ${president} Vote Share:<br />` + parseFloat(props["Past_Election_Results"][`${winner}`]).toFixed(2) + '%</b> <br />' + `</b> ${category} ${runner_up}  Vote Share:<br />` + parseFloat(props["Past_Election_Results"][`${loser}`]).toFixed(2) + '%</b> <br />'
        : 'Hover over a state');
        };
