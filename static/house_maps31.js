...
if (category == 2018) {
 ...
 d3.json(query_url, function(data) {
    ...
    this._div.innerHTML = '<h4> Election Results by House District</h4>' +  (props ?
        '<b>' + props.district + "<br>" + `</b> Pct of ${props.district} that is ${president}:<br />` + parseFloat(props[`${winner}`]).toFixed(2) + '%</b> <br />' + `</b> ${category} ${runner_up}  Vote Share:<br />` + parseFloat(props[`${loser}`]).toFixed(2) + '%</b> <br />'
        : 'Hover over a state');
        };
    ...
    function style(feature) {
        ...
        //getColor(feature.properties[`${year}`])
            };
